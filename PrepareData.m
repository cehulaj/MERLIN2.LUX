...
%     'LScaleFactor': Length scale factor (Ls)                   % Modified
...
if ~isfield(AnalyInputOpt,'StopCriterion')
    AnalyInputOpt.StopCriterion = @(Node,U,icrm,lmd)false;
end
...
if strcmpi(AnalyInputOpt.ModelType,'N4B5')
    ...
    % Modified
    pf02 = zeros(size(Fold,1),1);
    pf02 = pf02+pi;
    pb02 = zeros(size(Bend,1),1);
    pb02 = pb02+pi;
    ...
    % Modified
    Kb2 = getfieldvalues(AnalyInputOpt,'Kb2',kpb(1));
    if (size(Kb2,1)==1)&&(size(Bend,1)>1)
        Kb2 = repmat(Kb2,size(Bend,1),1);
    end
    Kf2 = getfieldvalues(AnalyInputOpt,'Kf2',kpf(1));
    if (size(Kf2,1)==1)&&(size(Fold,1)>1)
        Kf2 = repmat(Kf2,size(Fold,1),1);
    end
    ...
    % Modified
    truss.CM2 = getfieldvalues(AnalyInputOpt,'BarCM2',truss.CM);
    angles.CMbend2 = getfieldvalues(AnalyInputOpt,'RotSprBend2',@(he,h02,kb2,L0)EnhancedLinear(he,h02,kb2,L0,5,355));
    angles.CMfold2 = getfieldvalues(AnalyInputOpt,'RotSprFold2',@(he,h02,kf2,L0)EnhancedLinear(he,h02,kf2,L0,5,355));
    angles.Kb2 = Kb2;
    angles.Kf2 = Kf2;
    angles.pf02 = pf02;
    angles.pb02 = pb02;
...
elseif strcmpi(AnalyInputOpt.ModelType,'N5B8')
    ...
    if strcmpi(MaterCalib,'manual')
        ...
        % Modified
        EY2 = getfieldvalues(AnalyInputOpt,'ModElastic2',EY);
        truss.CM2 = getfieldvalues(AnalyInputOpt,'BarCM2',@(Ex2)Ogden(Ex2, EY2));
        ...
        % Modified
        Kf2 = zeros(size(Fold,1),1);
        Kb2 = zeros(size(Bend,1),1);
        
        G = EY*thck^3/(12*(1-nv^2));
        Lf = L((numel(kpb)+1):(numel(kpb)+numel(kpf)));
        % Rewritten: Ls = getfieldvalues(AnalyInputOpt,'LScaleFactor',2*sum(Lf)/numel(Lf));
        Ls = getfieldvalues(AnalyInputOpt,'LScaleFactor',50);
        % Rewritten: Kl = 1/Ls*G;
        Kl = Lf/Ls*G;  Km = 0.55*G*(Lf/thck).^(1/3);
        kpf = (1./(1./Kl+1./Km))./Lf;
        
        % Modified
        G2 = EY2*thck^3/(12*(1-nv^2));
        Ls2 = getfieldvalues(AnalyInputOpt,'LScaleFactor2',Ls);
        Kl2 = Lf/Ls2*G2;  Km2 = 0.55*G2*(Lf/thck).^(1/3);
        Kf2 = (1./(1./Kl2+1./Km2))./Lf;

        for j=1:length(panelctr)       
                [Abarj, kpbj] = GetMaterial(Node,Panel{j},panelctr(j),EY,nv,thck,Fold,Bend,Bdry);
                % Modified
                [ ~, kpbj2] = GetMaterial(Node,Panel{j},panelctr(j),EY2,nv,thck,Fold,Bend,Bdry);
                
                Abar(Abarj(:,1)) = Abar(Abarj(:,1))+Abarj(:,2);
                if ~isempty(kpbj)
                    kpb(kpbj(:,1)) = kpb(kpbj(:,1))+kpbj(:,2);
                    % Modified
                    Kb2(kpbj2(:,1)) = Kb2(kpbj2(:,1))+kpbj2(:,2);
                end
        end
        ...
        %Modified
        angles.CMbend2 = getfieldvalues(AnalyInputOpt,'RotSprBend2',@SuperLinearBend);
        angles.CMfold2 = getfieldvalues(AnalyInputOpt,'RotSprFold2',@(he,h02,kf2,L0)EnhancedLinear(he,h02,kf2,L0,5,355));
        angles.Kb2 = Kb2;
        angles.Kf2 = Kf2;

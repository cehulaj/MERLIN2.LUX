function [IF,K] = GlobalK_fast_ver2(Ui,UDef,Node,truss,angles)
...
Ex2 = truss.B2*Ui2./truss.L2+0.5*sum(du2.^2,2)./(truss.L2.^2);
[Sx2,Et2] = truss.CM2(Ex2);

Duelem2 = [du2,-du2];
Du2 = sparse(repmat(1:numel(Et2),1,6),eDofb(:),Duelem2(:),numel(Et2),numel(Ui2));
Fx2 = Sx2.*truss.A;
IFb2 = (sum(bsxfun(@times,truss.B2,Fx2.*truss.L./truss.L2),1)+sum(bsxfun(@times,Du2,Fx2.*truss.L./(truss.L2.^2)),1))';

Kel2 = truss.B2'*sparse(1:numel(Et2),1:numel(Et2),Et2.*truss.A.*truss.L./(truss.L2.^2))*truss.B2;
K12 = Du2'*sparse(1:numel(Et2),1:numel(Et2),Et2.*truss.A.*truss.L./(truss.L2.^3))*truss.B2+...
     truss.B2'*sparse(1:numel(Et2),1:numel(Et2),Et2.*truss.A.*truss.L./(truss.L2.^3))*Du2;
K22 = Du2'*sparse(1:numel(Et2),1:numel(Et2),Et2.*truss.A.*truss.L./(truss.L2.^4))*Du2;
G2 = sparse([1:numel(Et2),1:numel(Et2)],reshape(truss.Bars,[],1),[ones(numel(Et2),1);-ones(numel(Et2),1)],numel(Et2),Nn);
[ia2,ja2,sa2] = find(G2'*bsxfun(@times,G2,(Fx2.*truss.L./(truss.L2.^2))));
ik2 = bsxfun(@plus, 3*(ia2-1), 1:3); jk2 = bsxfun(@plus, 3*(ja2-1), 1:3);
Kg2 = sparse(ik2,jk2,repmat(sa2,[1,3]),3*Nn,3*Nn);
Kg2 = 0.5*(Kg2+Kg2');

Kb = (Kel+K1+K2)+Kg+(Kel2+K12+K22)+Kg2;
% Kb = (Kel)+Kg+(Kel2)+Kg2;
...
[Rsprb2, Ksprb2] = angles.CMbend2(he(1:size(angles.bend,1))',angles.pb02,angles.Kb2,truss.L(1:size(angles.bend,1)));
[Rsprf2, Ksprf2] = angles.CMfold2(he(size(angles.bend,1)+1:end)',angles.pf02,angles.Kf2,truss.L(size(angles.bend,1)+1:size(angles.bend,1)+size(angles.fold,1)));
Rspr(1:size(angles.bend,1)) = Rsprb+Rsprb2;
Kspr(1:size(angles.bend,1)) = Ksprb+Ksprb2;
Rspr(size(angles.bend,1)+1:end) = Rsprf+Rsprf2;
Kspr(size(angles.bend,1)+1:end) = Ksprf+Ksprf2;
